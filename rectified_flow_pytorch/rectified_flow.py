...
class RectifiedFlow(Module):
    def __init__(
        ...
        loss_fn: (
            Literal["mse", "pseudo_huber", "pseudo_huber_with_lpips"] | Module
        ) = "mse",
    ...
    def sample(
        ...
        **model_kwargs,
        ...
        assert not (
            use_ema and not self.use_consistency
...
class Trainer(Module):
    def __init__(
        ...
        *,
        ...
        ema_kwargs: dict = dict(),
        use_ema=True,
        num_train_steps_online: int = 10_000,
        steps: int = 16,
        verbose=True,
        fid_n_samples=3,
    ):
        ...
        self.dl_online = DataLoader(
            dataset, batch_size=batch_size, shuffle=True, drop_last=True
        )

        self.model, self.optimizer, self.dl, self.dl_online = self.accelerator.prepare(
            self.model, self.optimizer, self.dl, self.dl_online
        )

        self.num_train_steps = num_train_steps
        self.num_train_steps_online = num_train_steps_online
    ...
    def sample(self, model, fname, steps):
        eval_model = default(self.ema_model, model)
        dl = cycle(self.dl)
        mock_data = next(dl)
        ...
        with torch.no_grad():
            sampled = eval_model.sample(
                batch_size=self.num_samples, steps=steps, data_shape=data_shape
            )
        ...
        pbar = (
            tqdm(total=self.fid_n_samples, desc="Calculating FID")
            if self.verbose
            else None
        )

        for i in range(self.fid_n_samples):
            with torch.no_grad():
                sampled = eval_model.sample(
                    batch_size=1, steps=steps, data_shape=data_shape
                )
                real_batch = next(dl)

                # Calculate FID for each channel
                for c in range(3):
                    real_feature = real_batch[0, c, :, :].cpu().numpy()
                    sample_feature = sampled[0, c, :64, :64].cpu().numpy()
                    fid_channel = calculate_fid(real_feature, sample_feature)
                    fid += fid_channel / (3 * self.fid_n_samples)

            if self.verbose:
                pbar.update(1)
                pbar.set_postfix(fid=f"{fid:.4f}")
    ...
    def forward(self):
        ...
        pbar = (
            tqdm(total=self.num_train_steps, desc="Training") if self.verbose else None
        )
            ...
            if self.is_main:
                ...
                if divisible_by(step, self.save_results_every):
                    with torch.no_grad():
                        ...
                        save_image(
                            sampled,
                            str(
                                self.results_folder / f"results_{self.steps}.{step}.png"
                            ),
                        )
                ...
                if divisible_by(step, self.calculate_fid_every):
                    _, fid = self.sample(
                        self.model,
                        fname=str(
                            self.results_folder / f"fid_results_{self.steps}.{step}.png"
                        ),
                        steps=self.steps,
                    )
                    fid_ar.append(fid)
